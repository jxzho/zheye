import React, { Component } from 'react';
import { Table, Tag, message, Button } from 'antd';
import moment from "moment";
import api from '../../../api';

const columns = [
  {
    title: '账号',
    dataIndex: 'account',
    key: 'account',
  },
  {
    title: '昵称',
    dataIndex: 'user_info',
    key: 'user_info',
    render: (data) => {
      return data[0].nickname;
    }
  },
  {
    title: '权限',
    dataIndex: 'auth',
    key: 'auth',
    render: (text, record) => {
      if (text === 1) {
        return <Tag color="#108ee9">普通用户</Tag>
      } else {
        return <Tag color="#f50">管理员</Tag>
      }
    }
  },
  {
    title: '创建时间',
    dataIndex: 'createAt',
    key: 'createAt',
    render: (text, record) => moment(text).format("YYYY-MM-DD HH:mm")
  },
  {
    title: '状态',
    dataIndex: 'status',
    key: 'status',
    render: (text, record) => {
      if (text === 1) {
        return <Tag color="#87d068">正常</Tag>
      } else {
        return <Tag color="red">注销</Tag>
      }
    }
  },
  {
    title: '操作',
    dataIndex: 'action',
    key: 'action'
  }
];

class UserList extends Component {
  state = {
    data: []
  };

  render() {
    const { data } = this.state;
    return (
      <div className="user-edit-panel">
        <Table
          columns={columns}
          dataSource={data}
          rowKey={record => record.id}
        />
      </div>
    )
  }

  initColumns = () => {
    const item = columns.find(item => item.key === "action");
    item.render = (text, record) => {
      const auth = record.auth;
      const status = record.status;
      const id = record.id;
      return (
        <div className="btn-action-area">
          <Button
            type="link"
            size="small"
            onClick={() => {
              if (auth === 1) {
                api.updateUser(id, { auth: 2 }).then(res => {
                  this.initData();
                });
              } else {
                api.updateUser(id, { auth: 1 }).then(res => {
                  this.initData();
                });
              }
            }}
          >
            设为{auth === 1 ? "管理员" : "普通用户"}
          </Button>
          <Button type="danger" size="small" onClick={() => {
            if (status === 1) {
              api.updateUser(id, { status: 0 }).then(res => {
                this.initData();
              });
            } else {
              api.updateUser(id, { status: 1 }).then(res => {
                this.initData();
              });
            }
          }}>{status === 1 ? "注销" : "激活"}</Button>
        </div>
      )
    }
  }

  initData = () => {
    api.getUsers().then(res => {
      const data = res.data;
      if (data.result) {
        this.setState({
          data: data.data
        });
      } else {
        message.error(data.msg);
      }
    });
  };

  componentDidMount() {
    this.initData();
    this.initColumns();
  }
}

export default UserList;